                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: Math.min(totalPages, 3) }, (_, i) => i + 1).map(page => (
                    <Button
                      ...
                      className={`h-8 w-8 text-xs ${currentPage === page ? "bg-[#0F3460] text-white" : ""}`}
                    >
                      {page}
                    </Button>
                  ))}
  ...
  <DialogContent className="sm:max-w-[520px] rounded-2xl p-0">
    ...
    <div className="p-6 space-y-4">
      <div>
        ...
        <Input 
          placeholder="Dr. João Silva ou nome do paciente"
      ...
      <div>
        ...
        <Input 
          ...
          onChange={(e) => setNewPatient(prev => ({ ...prev, phone: formatPhoneInput(e.target.value) }))}
          className="h-11"
          maxLength={15}
        />
      </div>
      ...
      <div>
        <label className="text-sm font-medium text-[#1E293B] mb-1.5 block">Procedimento realizado</label>
        <Select value={newPatient.procedure} onValueChange={(val) => setNewPatient(prev => ({ ...prev, procedure: val }))}>
      ...
      <div>
        <label className="text-sm font-medium text-[#1E293B] mb-1.5 block">Valor pago</label>
        <div className="relative">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-[#64748B]">R$</span>
          <Input 
            type="number"
            placeholder="0"
            value={newPatient.value}
            onChange={(e) => setNewPatient(prev => ({ ...prev, value: e.target.value }))}
            className="h-12 pl-10 px-4"
          />
        </div>
      </div>
...
<Dialog open={showImportModal} onOpenChange={setShowImportModal}>
  <DialogContent className="sm:max-w-[520px] rounded-2xl p-0">
    <DialogHeader className="p-6 pb-4 border-b border-[#E2E8F0] flex items-center justify-between">
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="border-2 border-dashed border-[#E2E8F0] rounded-xl p-8 text-center hover:bg-[#F8FAFC] transition-colors">
        ...
        />
        {csvFile && (
          <p className="text-sm text-[#0F3460] mt-2 font-medium">{csvFile?.name}</p>
        )}
        {importSummary && (
          <div className="mt-4 p-3 bg-[#F0FDF4] rounded-lg text-sm">
            ...
            <p className="text-[#166534]">⚠️ {importSummary?.incompletos ?? 0} ignorados por dados incompletos</p>
            <p className="text-[#166534]">🔁 {importSummary?.duplicados ?? 0} ignorados por telefone duplicado</p>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-[#E2E8F0]">
        ...
        <Button
          ...
          disabled={!csvFile}
          className="flex-1 bg-[#0F3460] hover:bg-[#0D2A4D] text-white"
        >
          Importar pacientes
        </Button>
      </div>
